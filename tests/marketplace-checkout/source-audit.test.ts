...
describe("Phase 20 source contract", () => {
  ...
  it("keeps source production lock literal and environment-independent", () => { const lock = read("lib/marketplace-checkout/production-lock.ts"); expect(lock).toMatch(/MARKETPLACE_CHECKOUT_PRODUCTION_VALIDATION_APPROVED = false/); expect(lock).toMatch(/CHECKOUT_PUBLIC_DISABLED/); });
