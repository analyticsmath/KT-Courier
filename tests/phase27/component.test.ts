...
describe("Phase 27 component surfaces", () => {
  it("reuses one canonical notification centre across product roles", () => {
    ...
    for (const page of ["app/(account)/account/notifications/page.tsx", "app/(store)/store/notifications/page.tsx", "app/(driver)/driver/notifications/page.tsx", "app/(account)/promoter/notifications/page.tsx", "app/(applicant)/applicant/notifications/page.tsx"]) expect(readFileSync(join(process.cwd(), page), "utf8")).toContain("NotificationCentre");
  });
