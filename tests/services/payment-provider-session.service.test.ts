...
describe("payment provider session service", () => {
  ...
  it("reserves with a locked counter, calls the adapter outside transactions, and finalizes requires-action safely", async () => {
    ...
    expect(result).toMatchObject({ paymentStatus: "REQUIRES_ACTION", attempt: { attemptNumber: 1, status: "REQUIRES_ACTION", merchantReference: "kt-payment-pay=uabcdefghijklmnop-attempt-1" } });\n    expect(result.attempt.merchantReference).toMatch(/^[a-zA-Z0-9.=-]+$/); // Paystack-safe characters only
  ...
  it("leaves durable unresolved evidence when finalization rolls back", async () => {
    ...
    expect(attempt).toMatchObject({ status: "REQUESTING", merchantReference: "kt-payment-pay=uabcdefghijklmnop-attempt-1" });
  });
});
