...
const checks = [
  ...
  ["same payment has no second receipt journal", `SELECT COUNT(*)::int AS count FROM (SELECT p."id" FROM "Payment" p JOIN "LedgerJournal" j ON j."correlationId" = p."paymentNumber" WHERE j."type"::text = 'EXTERNAL_PAYMENT_RECEIPT' GROUP BY p."id" HAVING COUNT(*) > 1) d`],
  ["successful payments have one immutable verified-payment event", `SELECT COUNT(*)::int AS count FROM "Payment" p LEFT JOIN "PaymentVerifiedEventIntent" i ON i."paymentId" = p."id" WHERE p."status"::text = 'SUCCEEDED' AND (i."id" IS NULL OR i."successfulAttemptId" <> p."successfulAttemptId" OR i."webhookEventId" <> p."successWebhookEventId" OR i."amount" <> p."amount" OR i."currency" <> p."currency")`],
  ["verified-payment events remain derived from applied provider evidence", `SELECT COUNT(*)::int AS count FROM "PaymentVerifiedEventIntent" i LEFT JOIN "PaymentWebhookEvent" e ON e."id" = i."webhookEventId" WHERE e."id" IS NULL OR e."itnProcessingStatus"::text <> 'APPLIED' OR e."normalizedStatus"::text <> 'COMPLETE' OR NOT e."providerDataVerified"`],
