...
const checks = [
  ...
  ["no Payfast payment or attempt reference reached a ledger journal", `SELECT COUNT(*)::int AS count FROM "LedgerJournal" j WHERE EXISTS (SELECT 1 FROM "Payment" p LEFT JOIN "PaymentAttempt" a ON a."paymentId" = p."id" WHERE p."provider"::text = 'PAYFAST' AND (j."sourceReference" IN (p."publicReference", p."id", a."publicReference", a."merchantReference", a."id") OR j."correlationId" IN (p."publicReference", p."id", a."publicReference", a."merchantReference", a."id") OR COALESCE(j."metadata"::text, '') LIKE '%' || p."publicReference" || '%' OR COALESCE(j."metadata"::text, '') LIKE '%' || a."merchantReference" || '%'))`],
