...
import { NOTIFICATION_PROCESSOR_OPERATIONS, type NotificationProcessorOperation } from "@/lib/notifications/processor.service";
...
if (!NOTIFICATION_PROCESSOR_OPERATIONS.includes(operation as NotificationProcessorOperation)) throw new Error("Invalid Phase 27 processor operation.");

const composition = resolveNotificationProductionComposition();
try {
  const result = await composition.services.processors.run({ operation: operation as NotificationProcessorOperation, apply: mode === "--apply", limit });
