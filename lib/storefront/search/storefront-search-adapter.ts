...
function asMediaGallery(value: unknown): StorefrontDocument["mediaGallery"] {
  if (!Array.isArray(value)) return undefined;
  const items = value.flatMap((item) => {
    if (
      item &&
      typeof item === "object" &&
      typeof (item as { publicReference?: unknown }).publicReference === "string" &&
      ...
      return [{
        publicReference: (item as { publicReference: string }).publicReference,
        width: (item as { width: number }).width,
        height: (item as { height: number }).height,
        alt: (item as { alt: string }).alt,
        role: typeof (item as { role?: unknown }).role === "string" ? (item as { role: string }).role : undefined,
      }];
    }
    return [];
  });
