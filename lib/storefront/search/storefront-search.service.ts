...
export class StorefrontSearchService {
  ...
  async suggest(query: string) {
    ...
    return { normalizedQuery: normalized.value, products: unique(documents, (document) => document.productReference).slice(0, 5).map((document) => ({ productReference: document.productReference, productSlug: document.productSlug, title: document.title, variantReference: document.variantReference, ...(document.brandName ? { brandName: document.brandName } : {}), ...(document.primaryMedia ? { primaryMedia: document.primaryMedia } : {}) })), categories: unique(documents, (document) => document.categoryReference).slice(0, 3).map((document) => ({ reference: document.categoryReference, path: document.categoryPath })), brands: unique(documents.filter((document) => document.brandReference && document.brandName), (document) => document.brandReference!).slice(0, 3).map((document) => ({ reference: document.brandReference!, name: document.brandName! })), stores: unique(documents, (document) => document.storeReference).slice(0, 3).map((document) => ({ reference: document.storeReference, slug: document.storeSlug })), searchIndexVersion: STOREFRONT_SEARCH_INDEX_VERSION };
  }
}
