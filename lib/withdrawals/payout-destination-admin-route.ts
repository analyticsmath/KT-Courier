...
export async function preparePayoutDestinationMutation(request: NextRequest, params: Promise<{ id: string }>, path: string, actorUserId: string) {
  ...
  const rate = await checkIpRateLimit(request, `payout-destination:${actorUserId}`, RATE_LIMITS.PAYOUT_DESTINATION_MANAGE); if (!rate.ok) return { response: withdrawalNoStoreJson({ error: "Too many payout destination actions." }, 429) } as const;
