...
export function buildMarketplacePaystackCallbackUrls(
  appOrigin: string,
  paymentPublicReference: string,
): PaystackCallbackUrls {
  if (!PAYMENT_REFERENCE.test(paymentPublicReference)) {
    throw new PaymentError("PAYMENT_CALLBACK_URL_INVALID", "Payment callback reference is invalid.");
  }
  let origin: URL;
  ...
  if (
    (origin.protocol !== "https:" && !isHttpAllowed) ||
    origin.username ||
    origin.password ||
    origin.pathname !== "/" ||
    origin.search ||
  ...
  const query = `payment=${encodeURIComponent(paymentPublicReference)}`;
  return Object.freeze({
    returnUrl: new URL(`/checkout/paystack/return?${query}`, origin).toString(),
    cancelUrl: new URL(`/checkout/paystack/return?${query}&status=cancelled`, origin).toString(),
    notificationUrl: new URL("/api/payments/paystack/webhook", origin).toString(),
    returnRouteId: "paystack-return",
    cancelRouteId: "paystack-cancel",
    notificationRouteId: "paystack-webhook",
