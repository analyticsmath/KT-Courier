...
class NotConfiguredProvider implements NotificationProvider {
  ...
  async send(_input?: { destination: string; subject?: string; body: string; htmlBody?: string; idempotencyKey: string }): Promise<ProviderSendResult> { return { accepted: false, failureClass: this.failureClass, safeCode: this.name }; }
}
