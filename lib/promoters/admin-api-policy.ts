...
export async function requirePromoterAdmin(request: NextRequest, permission: string, path: string, mutate = false) {
  const rate = await checkIpRateLimit(request, path, { max: mutate ? 30 : 120, windowMs: 10 * 60 * 1000 }); if (!rate.ok) return { response: tooManyRequests(rate.retryAfterSeconds) } as const;
