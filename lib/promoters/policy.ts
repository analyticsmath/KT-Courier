...
export const PROMOTER_TARGET_TYPES = ["CUSTOMER", "BUSINESS_CUSTOMER", "STORE", "DRIVER"] as const;
/**
...
export function assertExactlyOneAttributionSubject(input: Readonly<{ customerUserId?: string | null; businessAccountId?: string | null; storeId?: string | null; driverProfileId?: string | null }>) {
  if ([input.customerUserId, input.businessAccountId, input.storeId, input.driverProfileId].filter(Boolean).length !== 1) throw new PromoterError("PROMOTER_INVALID_COMMAND", "An attribution must bind exactly one new acquisition subject.");
}

export function assertPromoterTargetAvailable(targetType: string): asserts targetType is "CUSTOMER" | "STORE" | "DRIVER" {
  ...
  if (targetType !== "CUSTOMER" && targetType !== "STORE" && targetType !== "DRIVER") throw new PromoterError("PROMOTER_INVALID_COMMAND", "Unsupported promoter program target.");
}
...
export function assertInternalDestination(destinationType: string, destinationReference?: string | null) {
  ...
  if (!/^(CUSTOMER_REGISTRATION|STORE_APPLICATION|DRIVER_APPLICATION)$/.test(destinationType) || (destinationReference && !/^[A-Za-z0-9._:-]{1,120}$/.test(destinationReference))) {
