import { CatalogInventoryTrackingMode, Prisma, StorefrontDocumentStatus } from "@prisma/client";
...
export class StorefrontProjectionService {
  async buildPublishedSnapshot(snapshotReference: string): Promise<StorefrontDocument> {
    ...
    const readyMedia = source.product.media
      .filter((item) => item.asset.status === "READY" && item.asset.privacyInspectionPassed && item.asset.width && item.asset.height && snapshot.media.some((m) => m.assetReference === item.asset.publicReference))
      .sort((a, b) => (a.role === "PRIMARY" ? -1 : b.role === "PRIMARY" ? 1 : a.displayOrder - b.displayOrder));
    const seenMedia = new Set<string>();
    const mediaGallery: Array<{ publicReference: string; width: number; height: number; alt: string; role: string }> = [];
    for (const item of readyMedia) {
      if (!seenMedia.has(item.asset.publicReference)) {
        seenMedia.add(item.asset.publicReference);
        mediaGallery.push({
          publicReference: item.asset.publicReference,
  ...
  private async persist(snapshotId: string, document: StorefrontDocument, productId: string, variantId: string, offerId: string, storeId: string, categoryId: string, priceVersionId: string) {
    ...
    const commonData = {
      publicationSnapshotId: snapshotId, publicationVersion: document.publicationVersion, productId, productPublicReference: document.productReference, productSlug: document.productSlug, productScope: document.productScope, variantId, variantPublicReference: document.variantReference, offerId, offerPublicReference: document.offerReference, storeId, storePublicReference: document.storeReference, storeSlug: document.storeSlug, categoryId, categoryPublicReference: document.categoryReference, categoryPath: document.categoryPath, productTypeCode: document.productTypeCode, productTypeVersion: document.productTypeVersion, brandPublicReference: document.brandReference ?? null, brandName: document.brandName ?? null, title: document.title, normalizedTitle: document.normalizedTitle, shortDescription: document.shortDescription ?? null, publicDescription: document.description ?? null, searchText: document.searchText, searchableAttributes: document.searchableAttributes as unknown as Prisma.InputJsonValue, filterableAttributes: document.filterableAttributes as unknown as Prisma.InputJsonValue, variantOptions: document.variantOptions as unknown as Prisma.InputJsonValue, condition: document.condition, fulfilmentMode: document.fulfilmentMode, sellingUnit: document.sellingUnit, priceVersionId, pricePublicReference: document.price.publicReference, priceAmount: document.price.amount, currency: "ZAR", priceIncludesTax: true, unitPriceAmount: document.price.unitAmount ?? null, unitPriceUnit: document.price.unit ?? null, unitPriceQuantity: document.price.quantity ?? null, inventoryTrackingMode, availabilityState: document.availability, primaryMediaPublicReference: document.primaryMedia?.publicReference ?? null, primaryMediaWidth: document.primaryMedia?.width ?? null, primaryMediaHeight: document.primaryMedia?.height ?? null, primaryMediaAlt: document.primaryMedia?.alt ?? null, mediaGallery: document.mediaGallery ? (document.mediaGallery as unknown as Prisma.InputJsonValue) : Prisma.JsonNull, searchable: true, indexable: false, status: StorefrontDocumentStatus.ACTIVE, publishedAt: new Date(document.publishedAt), sourceUpdatedAt: new Date(document.sourceUpdatedAt), indexedAt: new Date(), projectionVersion,
    };
