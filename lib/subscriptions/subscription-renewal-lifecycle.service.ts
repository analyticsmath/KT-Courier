  ...
  return Object.freeze({
    ...
    async prepareRenewal(input) {
      return database.$transaction(async (tx: any) => {
        ...
        const payment = await tx.payment.create({ data: { publicReference: ref("pay"), userId: cycle.contract.payerUserId, orderId: null, marketplaceCheckoutId: null, marketplaceOrderId: null, subscriptionInvoiceId: cycle.invoice.id, subjectType: "SUBSCRIPTION_INVOICE", provider: "PAYSTACK", purpose: "ORDER", status: "PROVIDER_PENDING", amount: cycle.invoice.total, currency: "ZAR", creationIdempotencyKey: `subscription-renewal:${cycle.id}`, creationRequestHash: `subscription-renewal-invoice:${cycle.invoice.id}`, latestAttemptNumber: 1, version: 1, metadata: { subjectType: "SUBSCRIPTION_INVOICE", invoiceReference: cycle.invoice.publicReference, renewal: true } } });
        const environment = process.env.PAYSTACK_ENVIRONMENT?.trim().toUpperCase() === "LIVE" ? "PRODUCTION" : "SANDBOX";
        await tx.paymentAttempt.create({ data: { paymentId: payment.id, publicReference: ref("pat"), attemptNumber: 1, provider: "PAYSTACK", idempotencyKey: `subscription-renewal-attempt:${cycle.invoice.id}`, requestHash: `subscription-renewal-attempt:${cycle.invoice.id}`, merchantReference: cycle.invoice.publicReference, status: "REQUESTING", amount: cycle.invoice.total, currency: "ZAR", providerEnvironment: environment, providerProtocolVersion: "paystack-recurring-v1", configurationFingerprint: `paystack-v1:${environment.toLowerCase()}`, providerCredentialVersion: "paystack-v1", startedAt: new Date(), version: 0 } });
