...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) { const origin = await enforceSameOriginRequest(request); if (origin) return origin; const user = await getCurrentUser(); if (!user) return unauthorized(); const parsed = schema.safeParse(await request.json().catch(() => null)); if (!parsed.success) return unprocessable("Preparation event validation failed."); try { return ok({ data: await recordVendorPreparation({ orderId: (await params).id, actorUserId: user.id, ...parsed.data, preparationDueAt: parsed.data.preparationDueAt ? new Date(parsed.data.preparationDueAt) : undefined }) }); } catch (error) { return badRequest(error instanceof ShippingObligationError ? error.code : "PREPARATION_EVENT_FAILED"); } }
