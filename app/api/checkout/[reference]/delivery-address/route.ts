...
import { updateMarketplaceCheckoutAddress } from "@/lib/marketplace-checkout/checkout.service";
...
export async function PUT(request: NextRequest, context: { params: Promise<{ reference: string }> }) {
  ...
  try {
    ...
    const checkout = await updateMarketplaceCheckoutAddress({
      ...
      owner,
      operation: {
        operationId: stringField(body, "operationId", 160),
        requestHash: stringField(body, "requestHash", 160),
        expectedVersion: integerField(body, "checkoutVersion"),
      },
      address: {
        recipientName: stringField(body, "recipientName", 120),
        line1: stringField(body, "line1", 180),
        line2: optional("line2", 180),
        suburb: optional("suburb", 100),
        city: stringField(body, "city", 100),
        province: stringField(body, "province", 100),
        postalCode: optional("postalCode", 24),
        deliveryInstructions: optional("deliveryInstructions", 500),
        serviceAreaReference: optional("serviceAreaReference", 120),
      },
    });
