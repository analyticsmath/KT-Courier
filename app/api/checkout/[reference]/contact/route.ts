...
import { updateMarketplaceCheckoutContact } from "@/lib/marketplace-checkout/checkout.service";
...
export async function PUT(request: NextRequest, context: { params: Promise<{ reference: string }> }) {
  ...
  try {
    ...
    assertExactKeys(body, ["recipientName", "email", "phone", "preferredContactMethod", "operationId", "requestHash", "checkoutVersion"]);
    const { reference } = await context.params;
    const checkout = await updateMarketplaceCheckoutContact({
      reference,
      owner,
      operation: {
      ...
      contact: {
        ...
        email: stringField(body, "email", 254).toLowerCase(),
        phone: stringField(body, "phone", 32),
