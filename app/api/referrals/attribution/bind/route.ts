...
export async function POST(request: NextRequest) {
  ...
  const rate = await checkIpRateLimit(request, "promoter-attribution-bind", { max: 10, windowMs: 10 * 60 * 1000 }); if (!rate.ok) return tooManyRequests(rate.retryAfterSeconds);
