...
export async function POST(request: NextRequest) {
  const rate = checkIpRateLimit(request, "promoter-referral-code-resolve", { max: 30, windowMs: 10 * 60 * 1000 }); if (!rate.ok) return tooManyRequests(rate.retryAfterSeconds);
