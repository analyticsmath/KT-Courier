...
import {
  ...
  createMarketplaceDeliveryBridge,
  generateStoreOrderPickupChallenge,
  markStoreOrderReadyForHandoff,
  proposeStoreOrderSubstitution,
  rejectMarketplaceStoreOrder,
  startStoreOrderPreparation,
...
export async function POST(request: NextRequest, context: { params: Promise<{ reference: string }> }) {
  try {
    ...
    if (action === "begin-review") { exactKeys(body, ["action", "operationId"]); result = await beginStoreOrderReview({ storeOrderReference: reference, actorUserId: actor.id, operationId, requestHash }); }
      ...
      exactKeys(body, ["action", "operationId", "preparationMinutes", "pickupInstructions"]);
      result = await acceptMarketplaceStoreOrder({ storeOrderReference: reference, actorUserId: actor.id, preparationMinutes: integer(body, "preparationMinutes"), pickupInstructions: text(body, "pickupInstructions", 1, 500), operationId, requestHash });
      try {
        const bridgeOpId = `${operationId}:bridge`;
        await createMarketplaceDeliveryBridge({ storeOrderReference: reference, actorUserId: actor.id, operationId: bridgeOpId, requestHash: hash("bridge-delivery", { operationId: bridgeOpId }) });
      } catch (bridgeError) {
        console.error("Auto delivery bridge after acceptance encountered non-blocking error:", bridgeError);
      }
    }
    else if (action === "bridge-delivery") { exactKeys(body, ["action", "operationId"]); result = await createMarketplaceDeliveryBridge({ storeOrderReference: reference, actorUserId: actor.id, operationId, requestHash }); }
