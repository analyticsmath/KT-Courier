...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { return ok(await reconcileCashCollection({ orderId: (await params).id, actorUserId: auth.user.id, receivedAmount: parsed.data.amount, operationId: parsed.data.operationId, evidenceReference: parsed.data.evidenceReference })); }
