...
import { transitionDriverStatus } from "@/lib/services/admin-drivers.service";
...
import { AdminDriverStatusChangeSchema } from "@/lib/validation/driver";
...
export async function PATCH(
  ...
  const parsed = AdminDriverStatusChangeSchema.safeParse(body);
  if (!parsed.success) {
    return unprocessable("Validation failed.", formatZodErrors(parsed.error.issues));
  }

  try {
    const updated = await transitionDriverStatus(session.id, id, parsed.data);
    return ok(updated);
  } catch (error) {
