...
export async function POST(request: NextRequest, { params }: { params: Promise<{ caseRef: string }> }) {
  ...
  // 4. Rate limiting
  const rateLimit = await checkIpRateLimit(request, "ad-reconciliation:retry-reversal", { max: 30, windowMs: 10 * 60 * 1000 });
  if (!rateLimit.ok) {
  ...
  try {
    ...
    if (!body.clickChargeId) {
      return unprocessable("clickChargeId is required.");
    }
    if (!body.operationId) {
    ...
    const result = await service.reverseInvalidAdvertisingClick({
      clickChargeId: body.clickChargeId,
      reason: body.reason || "Administrative click reversal",
      actorUserId: auth.user.id,
      operationId: body.operationId,
      requestHash: body.requestHash || `HASH-${body.operationId}`
    });

    return ok({ message: "Reversal retry completed", result });
  } catch (error) {
    return apiRouteError(error, { fallbackMessage: "Failed to retry click reversal.", domainErrorStatus: 422 });
  }
}
