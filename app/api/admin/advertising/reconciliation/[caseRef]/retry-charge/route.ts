...
export async function POST(request: NextRequest, { params }: { params: Promise<{ caseRef: string }> }) {
  ...
  // 4. Rate limiting
  const rateLimit = await checkIpRateLimit(request, "ad-reconciliation:retry-charge", { max: 30, windowMs: 10 * 60 * 1000 });
  if (!rateLimit.ok) {
  ...
  try {
    ...
    if (!body.measurementEventId) {
      return unprocessable("measurementEventId is required.");
    }
    if (!body.operationId) {
      return unprocessable("operationId is required.");
    }

    const service = new AdvertisingBillingService();
    const result = await service.chargeValidAdvertisingClick({
      measurementEventId: body.measurementEventId,
      sessionFingerprint: body.sessionFingerprint || "unknown",
      operationId: body.operationId,
      requestHash: body.requestHash || `HASH-${body.operationId}`
    });

    return ok({ message: "Charge retry completed", result });
  } catch (error) {
    return apiRouteError(error, { fallbackMessage: "Failed to retry click charge.", domainErrorStatus: 422 });
  }
}
