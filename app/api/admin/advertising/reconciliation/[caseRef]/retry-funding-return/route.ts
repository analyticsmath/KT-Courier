...
export async function POST(request: NextRequest, { params }: { params: Promise<{ caseRef: string }> }) {
  ...
  try {
    ...
    const result = await service.returnUnusedAdvertisingFunding({
      campaignVersionId: body.campaignVersionId || existing.campaignVersionId,
    ...
    return ok({ message: "Funding return retry completed", result });
  } catch (error) {
    return apiRouteError(error, { fallbackMessage: "Failed to retry funding return.", domainErrorStatus: 422 });
  }
}
