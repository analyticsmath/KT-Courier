...
export async function POST(request: NextRequest, context: { params: Promise<{ reference: string }> }) {
  const auth = await managedMarketingAdmin(request, PERMISSIONS.MANAGED_MARKETING_PACKAGES_RETIRE, true);
  if ("response" in auth) return auth.response;
  const { reference } = await context.params;
  try { return NextResponse.json({ packageVersion: await managedMarketingService.retirePackage(reference, { actorUserId: auth.user.id, actorRole: auth.user.role }) }); }
