...
export async function POST(request: NextRequest, context: { params: Promise<{ reference: string }> }) {
  ...
  try { return NextResponse.json({ placement: await managedMarketingService.setPlacementActive(reference, false, { actorUserId: auth.user.id, actorRole: auth.user.role }) }); }
