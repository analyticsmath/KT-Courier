...
export async function POST(request: NextRequest, context: { params: Promise<{ reference: string }> }) {
  const auth = await managedMarketingAdmin(request, PERMISSIONS.MANAGED_MARKETING_REQUESTS_PAUSE, true);
  if ("response" in auth) return auth.response;
  const body = await parseBody(request, reviewActionSchema); if (isRouteResponse(body)) return body;
  const { reference } = await context.params;
  try { return NextResponse.json({ request: await managedMarketingService.resumeRequest({ actorUserId: auth.user.id, actorRole: auth.user.role }, reference, body.operationId, body.note) }); }
