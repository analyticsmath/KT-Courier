...
import { endAdminCampaign } from "@/lib/promotions/admin-promotions.service";
...
export async function POST(
  ...
  try {
    ...
    const campaign = await endAdminCampaign(params.reference);
    return NextResponse.json(campaign, { headers: { "Cache-Control": "private, no-store, max-age=0" } });
  } catch {
    return errorResponse("End failed", 422);
  }
}
