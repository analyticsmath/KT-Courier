...
export async function POST(
  ...
  try {
    ...
    await requirePermission({ userId: user.id, role: user.role, permissionKey: PERMISSIONS.RECRUITMENT_HANDOFFS_PROCESS });

    const { reference } = await params;
    const service = new OnboardingHandoffService(prisma);
    const processed = await service.processHandoff(reference);
