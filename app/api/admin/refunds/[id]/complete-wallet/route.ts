...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const prepared = await prepareAdminRefundMutation(request, params, "/api/admin/refunds/[id]/complete-wallet", auth.user.id); if ("response" in prepared) return prepared.response;
  ...
  try { const refund = await completeRefundToCustomerWallet({ actorUserId: auth.user.id, publicReference: prepared.publicReference, operationId: parsed.data.operationId }); return refundNoStoreJson({ refund: { publicReference: refund.publicReference, status: refund.status } }); }
