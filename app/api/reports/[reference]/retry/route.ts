...
export async function POST(
  ...
  if (job.requesterUserId === session.id) await authorizeReportDefinition(session, definition, "READ");
    ...
    isAdmin = await hasPermission({ userId: session.id, role: session.role, permissionKey: PERMISSIONS.REPORT_JOB_RETRY });
  } else return forbidden("You do not have access to this report job.");

  try {
    const updated = await jobService.retryJob(reference, session.id, isAdmin);
    return ok({ data: updated });
  } catch {
    return badRequest("Report job could not be retried.");
  }
}
