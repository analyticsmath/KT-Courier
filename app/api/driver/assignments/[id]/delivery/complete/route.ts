...
import { completeDelivery } from "@/lib/services/delivery-execution.service";
...
export async function POST(
  ...
  const rl = await checkIpRateLimit(req, `delivery:complete:${ip}`, RATE_LIMITS.DELIVERY_COMPLETE);
  ...
  try {
    const result = await completeDelivery(assignmentId, driverProfileId, user.id, parsed.data);
    if (!result.ok) return conflict(result.error);
    return ok({ ...result.assignment, operationResult: result.operationResult ?? null });
  } catch (err) {
    console.error("[driver/assignments/[id]/delivery/complete POST]", err);
    return serverError();
  }
}
