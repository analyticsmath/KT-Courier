...
import { recordDeliveryAttempted } from "@/lib/services/delivery-execution.service";
...
export async function POST(
  ...
  const rl = await checkIpRateLimit(req, `delivery:attempted:${ip}`, RATE_LIMITS.DELIVERY_ATTEMPTED);
  ...
  const parsed = DeliveryAttemptedSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const result = await recordDeliveryAttempted(assignmentId, driverProfileId, user.id, parsed.data);
    if (!result.ok) return conflict(result.error);
    return ok({ ...result.assignment, operationResult: result.operationResult ?? null });
  } catch (err) {
    console.error("[driver/assignments/[id]/delivery/attempt POST]", err);
    return serverError();
  }
}
