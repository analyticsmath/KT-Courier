...
import { DeliveryFailedSchema } from "@/lib/validation/delivery";
import { getDriverProfileIdForUser } from "@/lib/services/driver-assignments.service";
import { recordDeliveryFailed } from "@/lib/services/delivery-execution.service";
...
export async function POST(
  ...
  const rl = await checkIpRateLimit(req, `delivery:failed:${ip}`, RATE_LIMITS.DELIVERY_FAILED);
  ...
  try {
    const result = await recordDeliveryFailed(assignmentId, driverProfileId, user.id, parsed.data);
    if (!result.ok) return conflict(result.error);
    return ok(result.assignment);
  } catch (err) {
    console.error("[driver/assignments/[id]/delivery/fail POST]", err);
    return serverError();
  }
}
