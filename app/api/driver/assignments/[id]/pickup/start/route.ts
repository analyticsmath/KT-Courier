...
import { StartPickupSchema } from "@/lib/validation/pickup";
import { getDriverProfileIdForUser } from "@/lib/services/driver-assignments.service";
import { startPickup } from "@/lib/services/pickup-custody.service";
...
export async function POST(
  ...
  const rl = await checkIpRateLimit(req, `pickup:start:${ip}`, RATE_LIMITS.PICKUP_START);
  ...
  const parsed = StartPickupSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const result = await startPickup(assignmentId, driverProfileId, user.id, parsed.data);
    if (!result.ok) return conflict(result.error);
    return ok({ ...result.assignment, operationResult: result.operationResult ?? null });
  } catch (err) {
    console.error("[driver/assignments/[id]/pickup/start POST]", err);
    return serverError();
  }
}
