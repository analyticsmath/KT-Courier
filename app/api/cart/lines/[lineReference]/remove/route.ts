...
export async function POST(request: NextRequest, context: { params: Promise<{ lineReference: string }> }) {
  ...
  try {
    ...
    assertExactKeys(body, ["cartReference", "cartVersion", "operationId", "requestHash"]);
    const repository = createPrismaMarketplaceCartRepository();
    const cart = await repository.lockCartByOwner(owner);
    ...
    const result = await removeCartLine(repository, {
      ...
      mutation: {
        ...
        expectedVersion: integerField(body, "cartVersion"),
      },
    });
