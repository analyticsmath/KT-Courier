...
import { listStorefrontCategories, listStorefrontCollections, listStorefrontStores } from "@/lib/services/storefront-catalog.service";
import { marketplaceCategoryHref, marketplaceCollectionHref, marketplaceProductHref, marketplaceStoreHref, marketplaceVariantHref } from "@/lib/public-marketplace/routes";
...
export default async function sitemap({ id }: { id: Promise<string> }): Promise<MetadataRoute.Sitemap> {
  if (!storefrontPublicExposureAllowed()) return [];
  ...
  if (segment === "collections") return (await listStorefrontCollections()).flatMap((collection) => { const href = marketplaceCollectionHref(collection.slug); return href ? [{ url: `${BASE_URL}${href}`, changeFrequency: "weekly" as const }] : []; });
