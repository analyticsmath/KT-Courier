...
export function StoreOverviewPage({ storeName, storeStatus, queue, recentCourierOrders, pickupConfigured, payableBalance, activity }: { storeName: string; storeStatus: string | null; queue: StoreFulfilmentQueue; recentCourierOrders: readonly StoreCourierOrderRecord[]; pickupConfigured: boolean; payableBalance: string | null; activity?: React.ReactNode }) {
  ...
  return <div className={styles.scope}><ProtectedPageFrame>
    ...
    <ProtectedContentGrid contextRail={<div className="space-y-4"><IllustrationFrame role="store" /><OperationalPanel title="Store state" padding="compact"><ProtectedStatus label={state.label} tone={state.tone} /><p className="mt-3 text-sm text-[var(--eo-text-secondary)]">{state.description}</p><Link className="eo-text-link mt-3 inline-flex" href="/store/profile">Review store details</Link></OperationalPanel><OperationalPanel title="Collection point" padding="compact"><p className="text-sm text-[var(--eo-text-secondary)]">{pickupConfigured ? "A saved pickup address is available for new delivery requests." : "A saved pickup address has not been configured."}</p><Link className="eo-text-link mt-3 inline-flex" href="/store/profile">{pickupConfigured ? "Manage pickup address" : "Add pickup address"}</Link></OperationalPanel>{payableBalance !== null ? <OperationalPanel title="Confirmed store balance" padding="compact"><p className="eo-store-money">ZAR {payableBalance}</p><p className="mt-2 text-sm text-[var(--eo-text-secondary)]">Confirmed earnings available in your store balance.</p><Link className="eo-text-link mt-3 inline-flex" href="/store/earnings">View earnings</Link></OperationalPanel> : null}</div>}>
