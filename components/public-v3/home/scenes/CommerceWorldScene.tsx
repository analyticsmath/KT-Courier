...
import { commerceChapterBudgetVh } from "../director/home-chapters";
...
export function CommerceWorldScene({ categories, stores, products, selectedCategoryId, onCategorySelectionChange, selectedStoreId, onStoreSelectionChange, selectedProductId, onProductSelectionChange }: {
  ...
  return (
    <section className={`${styles.chapter} ${styles.commerce}`} data-kt-scene="commerce" aria-labelledby="commerce-heading" style={{ minHeight: `${commerceChapterBudgetVh(stores.length)}svh` }}>
      <div className={styles.sticky} data-home-sticky-stage>
        <div className={styles.commerceViewport}>
          ...
          {hasStoreWorld && activeStore ? <div className={`${styles.commerceWorld} ${styles.storeWorld}`} data-commerce-world="stores" data-commerce-stores>
            ...
            <div className={styles.storeVisualField}>{storeVisuals.map((store, index) => <div key={store.id} className={styles.storeMedia} data-commerce-store-media data-active={index === 0} data-store-distance={index}><img src={store.image} alt={`${store.name} storefront`} /></div>)}<div className={styles.storeInfo}><h3>{activeStore.name}</h3><p>{activeStore.publishedOfferCount} published products</p><Link href={activeStore.href}>Visit Store <span aria-hidden="true">↗</span></Link></div></div>
          </div> : null}
