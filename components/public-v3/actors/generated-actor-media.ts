/**
 * AUTOGENERATED ACTOR PERFORMANCE METADATA
 * Generated by scripts/media/build-actor-performance-manifest.mjs
 * Authoritative source: Local raw PNG masters via sharp.metadata() & pixel difference analysis.
 * DO NOT EDIT MANUALLY.
 */

export interface GeneratedActorState {
  id: string;
  actorType: "white-truck" | "van" | "courier" | "red-truck";
  sourceFile: string;
  webpSrc: string;
  width: number;
  height: number;
  aspectRatio: number;
  hasAlpha: boolean;
  direction: "left" | "right" | "front" | "rear" | "top-down" | "turning" | "detail" | "center";
  action: "idle" | "approach" | "travel" | "accelerate" | "brake" | "open" | "load" | "handoff" | "turn" | "depart" | "carry";
  family: string;
  groundContact: {
    x: number;
    y: number;
  };
}

export const VAN_DOOR_CALIBRATION = {
  rawBounds: {
  "minX": 268,
  "maxX": 1107,
  "minY": 157,
  "maxY": 904,
  "width": 1448,
  "height": 1086
},
  normalizedMask: {
  "x": 0.1851,
  "y": 0.1446,
  "width": 0.5794,
  "height": 0.6878
},
  clipPathInset: "inset(14.46% 23.55% 16.76% 18.51%)",
} as const;

export const GENERATED_ACTOR_STATES: Record<string, GeneratedActorState> = {
  "white-truck:side-right": {
    id: "side-right",
    actorType: "white-truck",
    sourceFile: "01_full_side_view_facing_right.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-side-right.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "right",
    action: "travel",
    family: "hero",
    groundContact: { x: 0.142, y: 0.8002 },
  },
  "white-truck:side-left": {
    id: "side-left",
    actorType: "white-truck",
    sourceFile: "02_full_side_view_facing_left.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-side-left.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "left",
    action: "travel",
    family: "network",
    groundContact: { x: 0.8565, y: 0.7938 },
  },
  "white-truck:centered-hero": {
    id: "centered-hero",
    actorType: "white-truck",
    sourceFile: "03_centered_hero_side_view.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-centered-hero.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "right",
    action: "idle",
    family: "hero",
    groundContact: { x: 0.8565, y: 0.7938 },
  },
  "white-truck:front-3q-right": {
    id: "front-3q-right",
    actorType: "white-truck",
    sourceFile: "04_front_three_quarter_view_facing_right.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-front-3q-right.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "right",
    action: "approach",
    family: "road",
    groundContact: { x: 0.6799, y: 0.8398 },
  },
  "white-truck:front-3q-left": {
    id: "front-3q-left",
    actorType: "white-truck",
    sourceFile: "05_front_three_quarter_view_facing_left.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-front-3q-left.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "left",
    action: "approach",
    family: "road",
    groundContact: { x: 0.3104, y: 0.8812 },
  },
  "white-truck:rear-3q-right": {
    id: "rear-3q-right",
    actorType: "white-truck",
    sourceFile: "06_rear_three_quarter_view_facing_right.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-rear-three-quarter-view-facing-right.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "right",
    action: "depart",
    family: "road",
    groundContact: { x: 0.3122, y: 0.7965 },
  },
  "white-truck:rear-3q-left": {
    id: "rear-3q-left",
    actorType: "white-truck",
    sourceFile: "07_rear_three_quarter_view_facing_left.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-rear-three-quarter-view-facing-left.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "left",
    action: "depart",
    family: "road",
    groundContact: { x: 0.6606, y: 0.8702 },
  },
  "white-truck:top-down-straight": {
    id: "top-down-straight",
    actorType: "white-truck",
    sourceFile: "08_top_down_view.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-top-down-straight.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "top-down",
    action: "travel",
    family: "route",
    groundContact: { x: 0.0942, y: 0.712 },
  },
  "white-truck:top-down-angled": {
    id: "top-down-angled",
    actorType: "white-truck",
    sourceFile: "09_top_down_angled_straight_road.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-top-down-angled-straight-road.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "top-down",
    action: "approach",
    family: "route",
    groundContact: { x: 0.1708, y: 0.9745 },
  },
  "white-truck:wide-hero": {
    id: "wide-hero",
    actorType: "white-truck",
    sourceFile: "10_oversized_wide_hero_composition.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-oversized-wide-hero-composition.webp",
    width: 1916,
    height: 821,
    aspectRatio: 2.3337,
    hasAlpha: true,
    direction: "right",
    action: "travel",
    family: "hero",
    groundContact: { x: 0.8499, y: 0.8575 },
  },
  "white-truck:front-cab-close": {
    id: "front-cab-close",
    actorType: "white-truck",
    sourceFile: "11_close_crop_front_cab_only.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-close-crop-front-cab-only.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "right",
    action: "travel",
    family: "detail",
    groundContact: { x: 0.3529, y: 0.9853 },
  },
  "white-truck:cargo-box-close": {
    id: "cargo-box-close",
    actorType: "white-truck",
    sourceFile: "12_close_crop_long_cargo_box_only.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-cargo-box-material.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "right",
    action: "travel",
    family: "detail",
    groundContact: { x: 0.7082, y: 0.8714 },
  },
  "white-truck:rear-portion-close": {
    id: "rear-portion-close",
    actorType: "white-truck",
    sourceFile: "13_close_crop_rear_portion_only.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-close-crop-rear-portion-only.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "right",
    action: "travel",
    family: "detail",
    groundContact: { x: 0.6198, y: 0.9355 },
  },
  "white-truck:rear-doors-open": {
    id: "rear-doors-open",
    actorType: "white-truck",
    sourceFile: "14_rear_doors_slightly_open.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-rear-doors-slightly-open.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "rear",
    action: "open",
    family: "detail",
    groundContact: { x: 0.7838, y: 0.7201 },
  },
  "white-truck:motion-energy": {
    id: "motion-energy",
    actorType: "white-truck",
    sourceFile: "15_subtle_motion_energy.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-subtle-motion-energy.webp",
    width: 1916,
    height: 821,
    aspectRatio: 2.3337,
    hasAlpha: true,
    direction: "right",
    action: "accelerate",
    family: "hero",
    groundContact: { x: 0.1587, y: 0.8283 },
  },
  "white-truck:top-down-turning": {
    id: "top-down-turning",
    actorType: "white-truck",
    sourceFile: "16_top_down_turning_curve_transition.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-white-top-down-turning.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "turning",
    action: "turn",
    family: "route",
    groundContact: { x: 0.1041, y: 0.966 },
  },
  "van:side-right": {
    id: "side-right",
    actorType: "van",
    sourceFile: "01_full_side_view_facing_right.png",
    webpSrc: "/media/public/protagonists/protagonist-van-side-right.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "right",
    action: "travel",
    family: "street-right",
    groundContact: { x: 0.5121, y: 0.8204 },
  },
  "van:side-left": {
    id: "side-left",
    actorType: "van",
    sourceFile: "02_full_side_view_facing_left.png",
    webpSrc: "/media/public/protagonists/protagonist-van-full-side-view-facing-left.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "left",
    action: "travel",
    family: "collection",
    groundContact: { x: 0.7997, y: 0.8177 },
  },
  "van:front-3q-right": {
    id: "front-3q-right",
    actorType: "van",
    sourceFile: "03_front_three_quarter_facing_right.png",
    webpSrc: "/media/public/protagonists/protagonist-van-front-three-quarter-facing-right.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "right",
    action: "approach",
    family: "street-right",
    groundContact: { x: 0.5625, y: 0.8812 },
  },
  "van:front-3q-left": {
    id: "front-3q-left",
    actorType: "van",
    sourceFile: "04_front_three_quarter_facing_left.png",
    webpSrc: "/media/public/protagonists/protagonist-van-front-three-quarter-facing-left.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "left",
    action: "approach",
    family: "collection",
    groundContact: { x: 0.3436, y: 0.8499 },
  },
  "van:rear-3q-right": {
    id: "rear-3q-right",
    actorType: "van",
    sourceFile: "05_rear_three_quarter_facing_right.png",
    webpSrc: "/media/public/protagonists/protagonist-van-rear-three-quarter-facing-right.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "right",
    action: "depart",
    family: "street-right",
    groundContact: { x: 0.4662, y: 0.8785 },
  },
  "van:rear-3q-left": {
    id: "rear-3q-left",
    actorType: "van",
    sourceFile: "06_rear_three_quarter_facing_left.png",
    webpSrc: "/media/public/protagonists/protagonist-van-rear-three-quarter-facing-left.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "left",
    action: "depart",
    family: "street-left",
    groundContact: { x: 0.5473, y: 0.8766 },
  },
  "van:centered-hero": {
    id: "centered-hero",
    actorType: "van",
    sourceFile: "07_centered_hero_version.png",
    webpSrc: "/media/public/protagonists/protagonist-van-centered-hero.webp",
    width: 1254,
    height: 1254,
    aspectRatio: 1,
    hasAlpha: true,
    direction: "left",
    action: "idle",
    family: "street-left",
    groundContact: { x: 0.8022, y: 0.7656 },
  },
  "van:front-half-close": {
    id: "front-half-close",
    actorType: "van",
    sourceFile: "08_close_crop_front_half.png",
    webpSrc: "/media/public/protagonists/protagonist-van-close-crop-front-half.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "detail",
    action: "idle",
    family: "detail",
    groundContact: { x: 0.2769, y: 0.9991 },
  },
  "van:rear-half-close": {
    id: "rear-half-close",
    actorType: "van",
    sourceFile: "09_close_crop_rear_half.png",
    webpSrc: "/media/public/protagonists/protagonist-van-close-crop-rear-half.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "detail",
    action: "idle",
    family: "detail",
    groundContact: { x: 0.5383, y: 0.9678 },
  },
  "van:sliding-door-open": {
    id: "sliding-door-open",
    actorType: "van",
    sourceFile: "10_side_sliding_door_open.png",
    webpSrc: "/media/public/protagonists/protagonist-van-sliding-door-open.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "left",
    action: "open",
    family: "collection",
    groundContact: { x: 0.8018, y: 0.825 },
  },
  "van:rear-doors-open": {
    id: "rear-doors-open",
    actorType: "van",
    sourceFile: "11_rear_doors_open.png",
    webpSrc: "/media/public/protagonists/protagonist-van-rear-doors-open.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "rear",
    action: "open",
    family: "loading",
    groundContact: { x: 0.5118, y: 0.9162 },
  },
  "van:all-doors-open": {
    id: "all-doors-open",
    actorType: "van",
    sourceFile: "12_side_and_rear_doors_open.png",
    webpSrc: "/media/public/protagonists/protagonist-van-all-doors-open.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "rear",
    action: "open",
    family: "loading",
    groundContact: { x: 0.4986, y: 0.849 },
  },
  "van:motion-transition": {
    id: "motion-transition",
    actorType: "van",
    sourceFile: "13_slight_motion_web_transition.png",
    webpSrc: "/media/public/protagonists/protagonist-van-slight-motion-web-transition.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "left",
    action: "approach",
    family: "collection",
    groundContact: { x: 0.4723, y: 0.8916 },
  },
  "van:service-card": {
    id: "service-card",
    actorType: "van",
    sourceFile: "14_service_card_category_version.png",
    webpSrc: "/media/public/protagonists/protagonist-van-service-card-category-version.webp",
    width: 1254,
    height: 1254,
    aspectRatio: 1,
    hasAlpha: true,
    direction: "right",
    action: "idle",
    family: "service",
    groundContact: { x: 0.7011, y: 0.752 },
  },
  "courier:hero-standing": {
    id: "hero-standing",
    actorType: "courier",
    sourceFile: "01_original_pose_refined.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-hero-standing.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "front",
    action: "idle",
    family: "portrait",
    groundContact: { x: 0.7028, y: 0.9993 },
  },
  "courier:full-body-one-parcel": {
    id: "full-body-one-parcel",
    actorType: "courier",
    sourceFile: "02_full_body_one_parcel.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-carry-one-parcel.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "front",
    action: "carry",
    family: "portrait",
    groundContact: { x: 0.5945, y: 0.9715 },
  },
  "courier:full-body-two-parcels": {
    id: "full-body-two-parcels",
    actorType: "courier",
    sourceFile: "03_full_body_two_parcels.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-full-body-two-parcels.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "front",
    action: "carry",
    family: "portrait",
    groundContact: { x: 0.5891, y: 0.9722 },
  },
  "courier:walk-right-one-parcel": {
    id: "walk-right-one-parcel",
    actorType: "courier",
    sourceFile: "04_walking_one_parcel_facing_right.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-walk-right-one-parcel.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "right",
    action: "travel",
    family: "street",
    groundContact: { x: 0.6738, y: 0.9544 },
  },
  "courier:walk-left-one-parcel": {
    id: "walk-left-one-parcel",
    actorType: "courier",
    sourceFile: "05_walking_one_parcel_facing_left.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-walking-one-parcel-facing-left.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "left",
    action: "travel",
    family: "arrival",
    groundContact: { x: 0.3717, y: 0.9565 },
  },
  "courier:walk-two-parcels": {
    id: "walk-two-parcels",
    actorType: "courier",
    sourceFile: "06_walking_two_parcels.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-walking-two-parcels.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "right",
    action: "travel",
    family: "street",
    groundContact: { x: 0.3619, y: 0.9608 },
  },
  "courier:ready-handover": {
    id: "ready-handover",
    actorType: "courier",
    sourceFile: "07_ready_to_handover_parcel.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-ready-handover.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "left",
    action: "handoff",
    family: "custody",
    groundContact: { x: 0.6373, y: 0.9829 },
  },
  "courier:extending-handoff": {
    id: "extending-handoff",
    actorType: "courier",
    sourceFile: "08_extending_parcel_for_handoff.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-extending-handoff.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "left",
    action: "handoff",
    family: "arrival",
    groundContact: { x: 0.5936, y: 0.9736 },
  },
  "courier:look-viewer-parcel": {
    id: "look-viewer-parcel",
    actorType: "courier",
    sourceFile: "09_looking_toward_viewer_with_parcel.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-looking-toward-viewer-with-parcel.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "front",
    action: "idle",
    family: "portrait",
    groundContact: { x: 0.5998, y: 0.9772 },
  },
  "courier:look-right-approach": {
    id: "look-right-approach",
    actorType: "courier",
    sourceFile: "10_looking_right_approaching_vehicle.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-approach-vehicle.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "right",
    action: "approach",
    family: "vehicle",
    groundContact: { x: 0.6208, y: 0.9686 },
  },
  "courier:look-left-approach": {
    id: "look-left-approach",
    actorType: "courier",
    sourceFile: "11_looking_left_approaching_vehicle.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-looking-left-approaching-vehicle.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "left",
    action: "approach",
    family: "collection",
    groundContact: { x: 0.6894, y: 0.9786 },
  },
  "courier:place-parcel": {
    id: "place-parcel",
    actorType: "courier",
    sourceFile: "12_placing_parcel_down.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-placing-parcel-down.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "front",
    action: "load",
    family: "loading",
    groundContact: { x: 0.7567, y: 0.9258 },
  },
  "courier:lift-parcel": {
    id: "lift-parcel",
    actorType: "courier",
    sourceFile: "13_lifting_parcel_up.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-lifting-parcel-up.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "left",
    action: "load",
    family: "collection",
    groundContact: { x: 0.6796, y: 0.9779 },
  },
  "courier:small-parcel-one-hand": {
    id: "small-parcel-one-hand",
    actorType: "courier",
    sourceFile: "14_small_parcel_one_hand.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-small-parcel-one-hand.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "front",
    action: "carry",
    family: "portrait",
    groundContact: { x: 0.5922, y: 0.98 },
  },
  "courier:medium-box-arm": {
    id: "medium-box-arm",
    actorType: "courier",
    sourceFile: "15_medium_box_under_arm.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-medium-box-under-arm.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "front",
    action: "carry",
    family: "portrait",
    groundContact: { x: 0.5789, y: 0.9757 },
  },
  "courier:empty-hands-hero": {
    id: "empty-hands-hero",
    actorType: "courier",
    sourceFile: "16_empty_hands_courier_hero.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-empty-hands-courier-hero.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "front",
    action: "idle",
    family: "portrait",
    groundContact: { x: 0.5918, y: 0.9836 },
  },
  "courier:forward-gesture": {
    id: "forward-gesture",
    actorType: "courier",
    sourceFile: "17_forward_gesture_with_parcel.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-forward-gesture-with-parcel.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "front",
    action: "handoff",
    family: "arrival",
    groundContact: { x: 0.6511, y: 0.9893 },
  },
  "courier:loading-unloading": {
    id: "loading-unloading",
    actorType: "courier",
    sourceFile: "18_loading_unloading_parcel.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-loading-unloading.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "left",
    action: "load",
    family: "collection",
    groundContact: { x: 0.7598, y: 0.9757 },
  },
  "courier:half-body-holding": {
    id: "half-body-holding",
    actorType: "courier",
    sourceFile: "19_half_body_holding_parcel.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-half-body-holding-parcel.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "front",
    action: "carry",
    family: "portrait",
    groundContact: { x: 0.5887, y: 0.9993 },
  },
  "courier:portrait-upper-body": {
    id: "portrait-upper-body",
    actorType: "courier",
    sourceFile: "20_close_crop_upper_body_portrait.png",
    webpSrc: "/media/public/protagonists/protagonist-courier-close-crop-upper-body-portrait.webp",
    width: 1122,
    height: 1402,
    aspectRatio: 0.8003,
    hasAlpha: true,
    direction: "detail",
    action: "idle",
    family: "detail",
    groundContact: { x: 0.5927, y: 0.9993 },
  },
  "red-truck:side-right": {
    id: "side-right",
    actorType: "red-truck",
    sourceFile: "01_full_side_view_facing_right.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-side-right.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "right",
    action: "travel",
    family: "freight",
    groundContact: { x: 0.9103, y: 0.7566 },
  },
  "red-truck:side-left": {
    id: "side-left",
    actorType: "red-truck",
    sourceFile: "02_full_side_view_facing_left.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-full-side-view-facing-left.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "left",
    action: "travel",
    family: "freight-left",
    groundContact: { x: 0.09, y: 0.7365 },
  },
  "red-truck:front-3q-right": {
    id: "front-3q-right",
    actorType: "red-truck",
    sourceFile: "03_front_three_quarter_view_facing_right.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-front-three-quarter-view-facing-right.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "right",
    action: "approach",
    family: "road",
    groundContact: { x: 0.6492, y: 0.8766 },
  },
  "red-truck:front-3q-left": {
    id: "front-3q-left",
    actorType: "red-truck",
    sourceFile: "04_front_three_quarter_view_facing_left.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-front-three-quarter-view-facing-left.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "left",
    action: "approach",
    family: "road",
    groundContact: { x: 0.2752, y: 0.8131 },
  },
  "red-truck:rear-3q-right": {
    id: "rear-3q-right",
    actorType: "red-truck",
    sourceFile: "05_rear_three_quarter_view_facing_right.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-rear-three-quarter-view-facing-right.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "right",
    action: "depart",
    family: "road",
    groundContact: { x: 0.3605, y: 0.8831 },
  },
  "red-truck:rear-3q-left": {
    id: "rear-3q-left",
    actorType: "red-truck",
    sourceFile: "06_rear_three_quarter_view_facing_left.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-rear-three-quarter-view-facing-left.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "left",
    action: "depart",
    family: "road",
    groundContact: { x: 0.7072, y: 0.8462 },
  },
  "red-truck:centered-hero": {
    id: "centered-hero",
    actorType: "red-truck",
    sourceFile: "07_centered_hero_version.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-centered-hero.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "center",
    action: "idle",
    family: "freight",
    groundContact: { x: 0.3864, y: 0.7492 },
  },
  "red-truck:cab-crop": {
    id: "cab-crop",
    actorType: "red-truck",
    sourceFile: "08_front_cab_close_crop.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-front-cab-close-crop.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "detail",
    action: "travel",
    family: "detail",
    groundContact: { x: 0.298, y: 0.9991 },
  },
  "red-truck:trailer-middle-crop": {
    id: "trailer-middle-crop",
    actorType: "red-truck",
    sourceFile: "09_trailer_middle_section_close_crop.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-trailer-middle-section-close-crop.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "detail",
    action: "travel",
    family: "detail",
    groundContact: { x: 0.1143, y: 0.8628 },
  },
  "red-truck:rear-trailer-crop": {
    id: "rear-trailer-crop",
    actorType: "red-truck",
    sourceFile: "10_rear_trailer_section_close_crop.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-rear-trailer-section-close-crop.webp",
    width: 1448,
    height: 1086,
    aspectRatio: 1.3333,
    hasAlpha: true,
    direction: "detail",
    action: "travel",
    family: "detail",
    groundContact: { x: 0.7179, y: 0.9622 },
  },
  "red-truck:motion-entry": {
    id: "motion-entry",
    actorType: "red-truck",
    sourceFile: "11_motion_version.png",
    webpSrc: "/media/public/protagonists/protagonist-truck-red-motion-version.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "right",
    action: "accelerate",
    family: "freight",
    groundContact: { x: 0.1047, y: 0.7556 },
  },
  "red-truck:12_trailer_curtain_partially_opened": {
    id: "12_trailer_curtain_partially_opened",
    actorType: "red-truck",
    sourceFile: "12_trailer_curtain_partially_opened.png",
    webpSrc: "/media/public/protagonists/unknown.webp",
    width: 1672,
    height: 941,
    aspectRatio: 1.7768,
    hasAlpha: true,
    direction: "center",
    action: "idle",
    family: "default",
    groundContact: { x: 0.2765, y: 0.7577 },
  },
};
